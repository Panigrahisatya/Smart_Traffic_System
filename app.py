...
def init_db():
    ...
    # Check and update accidents table if traffic_id is missing 
    cursor.execute("PRAGMA table_info(accidents)")
    columns = [col[1] for col in cursor.fetchall()]
    if 'traffic_id' not in columns:
        cursor.execute('ALTER TABLE accidents ADD COLUMN traffic_id INTEGER')
        cursor.execute('UPDATE accidents SET traffic_id = NULL')
        cursor.execute('PRAGMA foreign_keys=OFF')
        cursor.execute('CREATE TABLE temp_accidents AS SELECT * FROM accidents')
        ...
        ''')
        cursor.execute('INSERT INTO accidents (id, traffic_id, location, description, severity, timestamp) SELECT id, traffic_id, location, description, severity, timestamp FROM temp_accidents')
        cursor.execute('DROP TABLE temp_accidents')
        cursor.execute('PRAGMA foreign_keys=ON')
    ...
    # Default admin and user insertion 
...
def admin_dashboard():
    ...
    with sqlite3.connect("traffic.db") as conn:
        ...
        cursor.execute("SELECT id, traffic_id, location, description, severity, timestamp FROM accidents ORDER BY id DESC LIMIT 20")
        accidents = cursor.fetchall()  # Explicitly select timestamp
...
def chatbot():
    ...
    if location and location.lower() in cuttack_locations:
        ...
        if "traffic" in user_query:
            ...
            store_accident_data(
                location=location,  # Ensure location is from the query, not overridden
                description=accident_data["description"],  # Ensure description is correct
